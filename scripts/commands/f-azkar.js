...
module.exports.handleEvent = function ({ api, event }) {
	...
	return api.sendMessage(`⚔️ ${command.config.name} ⚔️\n${command.config.description}\n\n❯ استخدام: ${prefix}${command.config.name} ${(command.config.usages) ? command.config.usages : ""}\n❯ تنتمي إلى المجموعة: ${command.config.commandCategory}\n❯ وقت الانتظار: ${command.config.cooldowns} ثانية\n❯ قوة: ${((command.config.hasPermssion == 0) ? "مستخدم" : (command.config.hasPermssion == 1) ? "المسؤولين" : "مشغل بوت" )}\n❯ Prefix: ${prefix}\n\n» رمز الوحدة بواسطة ${command.config.credits} «`, threadID, messageID);
};
